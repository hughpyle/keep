...
---
# Tag: `kind` — Content Kind

The `kind` tag classifies an item by what kind of content it represents — what you got out of it. It answers "what kind of insight or artifact is this?" rather than what entity type it is (see `type`) or what speech act it performs (see `act`).
...
## Relationship to `type` and `act`

The `kind` tag describes the *content*. The `type` tag describes the *entity*. The `act` tag describes the *speech act*. They answer different questions:

- `kind=learning` — "What kind of content is this?" → A learning.
- `type=conversation` — "What is this entity?" → A conversation.
- `act=assertion` — "What is the speaker doing?" → Stating a fact.

A single item might be `type=conversation kind=learning act=assessment` — a conversation that yielded a learning, expressed as an evaluation.

## Injection

When `analyze --tags kind` is used, the full text of this doc is prepended to the analysis prompt as guide context. This tag is unconstrained — values are free-form, but the table above lists the standard vocabulary.
...
```bash
# Capture a learning
keep put "Mock time in tests instead of real sleep assertions" -t kind=learning -t topic=testing

# Record a breakdown
keep put "Assumed X, actually Y. Next time: Z" -t kind=breakdown -t project=myapp
...
keep put "sqlite WAL mode required for concurrent reads" -t kind=gotcha -t topic=database

# Record a decision
keep put "Chose LanceDB over Pinecone: local-first, no API dependency" -t kind=decision -t project=keep
```
